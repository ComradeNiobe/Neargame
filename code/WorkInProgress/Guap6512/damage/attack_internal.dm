...
/obj/item/proc/stab(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
	...
	msg_admin_attack("[user.name] ([user.ckey]) stabbed [M.name] ([M.ckey]) with [src] (INTENT: [uppertext(user.a_intent)]) (<A HREF='byond://?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)") //BS12 EDIT ALG
