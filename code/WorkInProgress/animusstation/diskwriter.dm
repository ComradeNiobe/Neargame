...
/obj/item/disk/music/tape1/New()
	..()
	name = "\"Enoch's Gate\'s Greatest Hits Vol.1\" magn-o-tape"
	a_side = 'sound/music/csrio.ogg'
	sound_inside = a_side

/obj/item/disk/music/tape2/New()
	..()
	name = "\"Enoch's Gate\'s Greatest Hits Vol.2\" magn-o-tape"
	a_side = 'sound/music/soufoda.ogg'
	sound_inside = a_side

/obj/item/disk/music/tape3/New()
	..()
	name = "\"Enoch's Gate\'s Greatest Hits Vol.3\" magn-o-tape"
			...
			if(N)
				...
				if(S)
					...
					if(istype(src, /obj/machinery/party/musicwriter/diabolic_machine))
						disk.bloqueado = 1
					message_admins("[M.real_name]([M.ckey]) uploaded <A HREF='byond://?_src_=holder;listensound=\ref[S]'>sound</A> named as [N]. <A HREF='byond://?_src_=holder;wipedata=\ref[disk]'>Wipe</A> data.")
			if(istype(src, /obj/machinery/party/musicwriter/diabolic_machine))
				icon_state = "diabolic"
			else
				icon_state = "writer_off"
