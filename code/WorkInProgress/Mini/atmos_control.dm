...
/obj/machinery/computer/atmoscontrol/proc/return_controls()
	...
	switch(current.screen)
		if (AALARM_SCREEN_MAIN)
			...
			output += {"
<a href='byond://?src=\ref[src];alarm=\ref[current];screen=[AALARM_SCREEN_SCRUB]'>Scrubbers Control</a><br>
<a href='byond://?src=\ref[src];alarm=\ref[current];screen=[AALARM_SCREEN_VENT]'>Vents Control</a><br>
<a href='byond://?src=\ref[src];alarm=\ref[current];screen=[AALARM_SCREEN_MODE]'>Set environmental mode</a><br>
<a href='byond://?src=\ref[src];alarm=\ref[current];screen=[AALARM_SCREEN_SENSORS]'>Sensor Control</a><br>
<HR>
"}
			if (current.mode==AALARM_MODE_PANIC)
				output += "<font color='red'><B>PANIC SYPHON ACTIVE</B></font><br><A href='byond://?src=\ref[src];alarm=\ref[current];mode=[AALARM_MODE_SCRUBBING]'>turn syphoning off</A>"
			else
				output += "<A href='byond://?src=\ref[src];alarm=\ref[current];mode=[AALARM_MODE_PANIC]'><font color='red'><B>ACTIVATE PANIC SYPHON IN AREA</B></font></A>"
...
"}
			...
			output = {"<a href='byond://?src=\ref[src];alarm=\ref[current];screen=[AALARM_SCREEN_MAIN]'>Main menu</a><br>[sensor_data]"}
...
<B>d_filtering:</B>
Carbon Dioxide
<A href='byond://?src=\ref[src];alarm=\ref[current];id_tag=[id_tag];command=co2_scrub;val=[!data["d_filter_co2"]]'>[data["d_filter_co2"]?"on":"off"]</A>;
Toxins
<A href='byond://?src=\ref[src];alarm=\ref[current];id_tag=[id_tag];command=tox_scrub;val=[!data["d_filter_toxins"]]'>[data["d_filter_toxins"]?"on":"off"]</A>;
Nitrous Oxide
<A href='byond://?src=\ref[src];alarm=\ref[current];id_tag=[id_tag];command=n2o_scrub;val=[!data["d_filter_n2o"]]'>[data["d_filter_n2o"]?"on":"off"]</A>
<BR>
"}
					sensor_data += {"
...
<b>Air machinery mode for the area:</b><ul>"}
			...
			for (var/m=1,m<=modes.len,m++)
				if (current.mode==m)
					...
					output += {"<li><A href='byond://?src=\ref[src];alarm=\ref[current];mode=[m]'>[modes[m]]</A></li>"}
			output += "</ul>"
