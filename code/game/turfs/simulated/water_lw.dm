...
/turf/simulated/floor/seafloor/Entered(A)
	if(ismob(A))
	...
	if(ismob(A))
		...
		if(M.client)
			...
			for(var/obj/I in M?.client?.usingPlanes)
				...
				I.add_filter("color", 21, list("type" = "color", "color" = "#365ea15d"))
				var/filter = I.get_filter("blur")
				screen_blur(filter, 1, 50)
		if(!M.AreasEntered.Find(src))
