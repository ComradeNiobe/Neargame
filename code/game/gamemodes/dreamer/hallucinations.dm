/mob/living/carbon/human
	...
	var/waking_up = FALSE
	var/list/seenwonders = list()

/mob/living/carbon/human/proc/handle_dreamer()
	if(fakeDREAMER && combat_mode)
		spawn(0)
			INVOKE_ASYNC(src, PROC_REF(handle_dreamer_screenshake))
	if(src && istype(src) && is_dreamer(src))
		if(combat_mode || waking_up)
			spawn(0)
				INVOKE_ASYNC(src, PROC_REF(handle_dreamer_screenshake))
		INVOKE_ASYNC(src, PROC_REF(handle_dreamer_hallucinations))
		...
		if(waking_up)
			INVOKE_ASYNC(src, PROC_REF(handle_dreamer_waking_up))
...
/mob/living/carbon/human/proc/handle_dreamer_hallucinations()
	...
	for(var/F in floorlist)
		INVOKE_ASYNC(src, PROC_REF(handle_dreamer_floor), F)

	for(var/W in walllist)
		INVOKE_ASYNC(src, PROC_REF(handle_dreamer_wall), W)
...
/mob/living/carbon/human/proc/handle_dreamer_waking_up()
	...
	for(var/turf/simulated/floor/F in view(src))
		if(prob(15))
			floorlist |= F
	for(var/F in floorlist)
		INVOKE_ASYNC(src, PROC_REF(handle_waking_up_floor), F)
