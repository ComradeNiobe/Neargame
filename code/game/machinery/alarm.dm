...
"}
			if (mode==AALARM_MODE_PANIC)
				output += "<font color='red'><B>PANIC SYPHON ACTIVE</B></font><br><A href='byond://?src=\ref[src];mode=[AALARM_MODE_SCRUBBING]'>Turn syphoning off</A>"
			else
				output += "<A href='byond://?src=\ref[src];mode=[AALARM_MODE_PANIC]'><font color='red'>ACTIVATE PANIC SYPHON IN AREA</font></A>"
...
<B>d_filtering:</B>
Carbon Dioxide
<A href='byond://?src=\ref[src];id_tag=[id_tag];command=co2_scrub;val=[!data["d_filter_co2"]]'>[data["d_filter_co2"]?"on":"off"]</A>;
Toxins
<A href='byond://?src=\ref[src];id_tag=[id_tag];command=tox_scrub;val=[!data["d_filter_toxins"]]'>[data["d_filter_toxins"]?"on":"off"]</A>;
Nitrous Oxide
<A href='byond://?src=\ref[src];id_tag=[id_tag];command=n2o_scrub;val=[!data["d_filter_n2o"]]'>[data["d_filter_n2o"]?"on":"off"]</A>
<BR>
"}
					sensor_data += {"
