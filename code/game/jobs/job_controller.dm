 ...
 **/
	...
	proc/PostEquip(var/mob/living/carbon/human/H, var/joined_late = 0, var/turf/pickNewmigLocs)
		...
		spawn(15 SECONDS)
			...
			// Shows the round aspect when they spawn in, in case they missed it or are latejoining
			if(ticker.eof.roundstartdisplay)
				to_chat(H, "<span class ='passivebold'>Praise the Lord!</span> <span class ='passive'>[ticker.eof.event_message]</span>")
