...
/datum/dna/GetCloneArgs()
	return list(unique_enzymes, b_type, mutantrace, real_name, species, SE, UI)

/datum/dna/PopulateClone(datum/dna/clone)
	clone = ..()
	clone.unique_enzymes = unique_enzymes
	clone.b_type = b_type
	clone.mutantrace = mutantrace
	clone.real_name = real_name
	clone.species = species
	clone.SE = listDeepClone(SE)
	clone.UI = listDeepClone(UI)
	return clone
