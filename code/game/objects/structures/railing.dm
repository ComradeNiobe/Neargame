	...
	if (istype(W, /obj/item/grab) && get_dist(src,user)<2)
		...
		if(G.state<2)
			if(ishuman(G.affecting))
				...
				message_admins("ATTACK: [G.assailant] ([G.assailant.ckey])(<A HREF='byond://?_src_=holder;adminplayerobservejump=\ref[G]'>JMP</A>) smashed [G.affecting] ([G.affecting.ckey]) on a [src].", 2)
