...
/obj/machinery/shower/proc/wash(atom/movable/O as obj|mob)
	...
	if(isliving(O))
		...
		L.on_fire = 0
		// Fun fact: this used to be unclamped, so the shower could just kill you within seconds.
		L.bodytemperature = clamp(L.bodytemperature-95, BODYTEMP_COLD_DAMAGE_LIMIT, BODYTEMP_HEAT_DAMAGE_LIMIT)
		if(iscarbon(O))
