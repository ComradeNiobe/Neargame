...
/obj/item/proc/QualityToThing(obj/item/A)
	switch(A)
		if(1)
			return " It's an average item."
		if(2)
			return " It's a quality item."
		if(3)
			return " It's an impressive item."
		if(4)
			return " It's an exceptional item."
		if(5)
			return " It's a truly mastercrafted item!"
		if(6)
			return " It's a work of art!"
...
/obj/item/attackby(obj/item/W as obj, mob/living/carbon/human/user as mob)
			...
			if(H.my_skills.get_skill(SKILL_SMITH) > 3)
				if(src.durability < 100)
					...
					switch(what)
						...
						if("Name")
							var/naem = sanitize_safe(input("What name?", "[src]")) as text
							if(length(naem) > 25)
	...
	if(istype(W, /obj/item/stone))
		...
		if(prob(12))
			...
			for(var/obj/F in oview(1))
				if(istype(F, /obj/item/flame/torch))
					var/obj/item/flame/torch/T = F
					T.turn_on()
					user.visible_message("<span class='passivebold'>The sparks</span> <span class='passive'>enlight \the Torch!</span>")
					if(F.loc == user.contents)
						user.on_fire = 1
						user.update_fire()
				if(istype(F, /obj/structure/fireplace))
					var/obj/structure/fireplace/T = F
					T.turn_on()
					user.visible_message("<span class='passivebold'>The sparks</span> <span class='passive'>enlight \the Fire!</span>")
				if(istype(F, /obj/structure/campfire))
					var/obj/structure/campfire/T = F
					if(!T.spent)
						T.on = TRUE
						T.checkfire()
...
/obj/item/proc/dropped(mob/user as mob)
	...
	if(user?.l_hand)
		...
		src.item_state = "[src::item_state]"
		if(blooded_icon)
			src.item_state = "[src::item_state][blood_suffix]"
	if(user?.r_hand)
