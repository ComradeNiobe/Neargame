	...
	if(href_list["order"])
		if(href_list["order"] == "categories")
			...
			for(var/supply_name in supply_shuttle.supply_packs )
				...
				temp += "<A href='byond://?src=\ref[src];doorder=[supply_name]'>[supply_name]</A> Cost: [N.cost]<BR>"		//the obj because it would get caught by the garbage
...
/obj/machinery/computer/supplycomp/RightClick(mob/living/carbon/human/user as mob)
	if(ishuman(user) && user.wear_id)
		...
		if(!istype(idcard, /obj/item/card/id/qmboss)) return
		if(locked)
			locked = FALSE
...
*/
		...
		for(var/S in supply_shuttle.requestlist)
			var/datum/supply_order/SO = S
			temp += "#[SO.ordernum] - [SO.object.name] requested by [SO.orderedby]  [supply_shuttle.moving ? "":supply_shuttle.at_station ? "":"<A href='byond://?src=\ref[src];confirmorder=[SO.ordernum]'>Approve</A> <A href='byond://?src=\ref[src];rreq=[SO.ordernum]'>Remove</A>"]<BR>"
