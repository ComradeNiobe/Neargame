...
/datum/organ/external
	...
	var/datum/organ/external/child_organs
...
/datum/organ/external/proc/handle_germ_effects()
	...
	if(germ_level >= INFECTION_LEVEL_TWO && antibiotics < 1)
		...
		if (children)
			for (child_organs in children)
				if (child_organs.germ_level < germ_level && !(child_organs.status & ORGAN_ROBOT))
					if (child_organs.germ_level < INFECTION_LEVEL_ONE*2 || prob(30))
						child_organs.germ_level++
...
/datum/organ/external/proc/setAmputatedTree()
	for(child_organs in children)
		child_organs.amputated = amputated
		child_organs.setAmputatedTree()
...
/datum/organ/external/proc/droplimb(var/override = 0,var/no_explode = 0, var/gibbed = 0, var/no_organ_item = FALSE)
	...
	if(status & ORGAN_DESTROYED)
		...
		for(child_organs as anything in children)
			if(child_organs.children)
				for(var/datum/organ/external/child_organs_recursive as anything in child_organs.children)
					child_organs_recursive.droplimb(1)
			child_organs.droplimb(1)
...
/datum/organ/external/proc/release_restraints()
	if(owner.handcuffed)
		if(body_part in list(ARM_LEFT, ARM_RIGHT, HAND_LEFT, HAND_RIGHT))
			owner.visible_message(\
				"\The [owner.handcuffed.name] falls off of [owner.name].",\
				"\The [owner.handcuffed.name] falls off you.")

			owner.drop_from_inventory(owner.handcuffed)
	else if(owner.legcuffed)
		if(body_part in list(FOOT_LEFT, FOOT_RIGHT, LEG_LEFT, LEG_RIGHT))
			owner.visible_message(\
...
/datum/organ/external/proc/is_usable()
	if(cripple_left > 0)
		return 0
	return !(status & (ORGAN_DESTROYED|ORGAN_MUTATED|ORGAN_DEAD|ORGAN_CUT_AWAY|ORGAN_TENDON) || !(painLW < pain_disability_threshold))
...
/datum/organ/external/proc/sever_artery()
	if(!(status & ORGAN_ARTERY) && src.parent)
		status |= ORGAN_ARTERY
		for(child_organs as anything in children)
			if(src?.child_organs?.status & ORGAN_ARTERY && src?.child_organs?.status & ORGAN_DESTROYED)
				src.child_organs.status &= ~ORGAN_ARTERY
