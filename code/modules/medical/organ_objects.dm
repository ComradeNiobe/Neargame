...
/obj/item/reagent_containers/food/snacks/organ/proc/removed(var/mob/living/target,var/mob/living/user)
	...
	if(organ_data.vital)
		...
		msg_admin_attack("[user.name] ([user.ckey]) removed a vital organ ([src]) from [target.name] ([target.ckey]) (INTENT: [uppertext(user.a_intent)]) (<A HREF='byond://?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)")
		target.death()
