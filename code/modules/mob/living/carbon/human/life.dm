...
/mob/living/carbon/human/Life()
	...
	*/
	...
	if(master_mode == "changeling")
		for(var/datum/reagent/C in reagents.reagent_list)
			if(C.id == ticker.mode.changeling_chem)
				if(mind.special_role == "Changeling" && tentaclesexposed == FALSE)
					extend_tentacles()
...
//				playsound_local(src,pick(scarySounds),50, 1, -1)
	...
	proc/handle_stomach()
		...
		if(hydration < THIRST_LEVEL_MEDIUM)
			...
			if(clamp(hydration, 150, THIRST_LEVEL_DEHYDRATED))
				if(prob(2))
			...
			if(clamp(hydration, THIRST_LEVEL_DEHYDRATED, -INFINITY))
