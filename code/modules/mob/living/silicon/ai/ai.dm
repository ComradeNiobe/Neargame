			...
			for (var/alarm in L)
				...
				if (C && istype(C, /list))
					...
					for (var/obj/machinery/camera/I in C)
						dat2 += text("[]<A HREF=byond://?src=\ref[];switchcamera=\ref[]>[]</A>", (dat2=="") ? "" : " | ", src, I, I.c_tag)
					dat += text("-- [] ([])", A.name, (dat2!="") ? dat2 : "No Camera")
				else if (C && istype(C, /obj/machinery/camera))
					var/obj/machinery/camera/Ctmp = C
					dat += text("-- [] (<A HREF=byond://?src=\ref[];switchcamera=\ref[]>[]</A>)", A.name, src, C, Ctmp.c_tag)
				else
	...
	if (O)
		if (C && C.can_use())
			...
			for (var/obj/machinery/camera/I in CL)
				dat2 += text("[]<A HREF=byond://?src=\ref[];switchcamera=\ref[]>[]</A>", (!foo) ? "" : " | ", src, I, I.c_tag)	//I'm not fixing this shit...
				foo = 1
