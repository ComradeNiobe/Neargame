/mob/Destroy()//This makes sure that mobs with clients/keys are not just deleted from the game.
	...
	QDEL_NULL(hud_used)
	if(attack_delayer)
		QDEL_NULL(attack_delayer)
	if(click_delayer)
		QDEL_NULL(click_delayer)
	if(clong_delayer)
		QDEL_NULL(clong_delayer)

	qdel(reagents)
	if(special_delayer)
		QDEL_NULL(special_delayer)
	if(throw_delayer)
		QDEL_NULL(throw_delayer)
	clear_fullscreen()
	if(client)
...
/*		if(/obj/item/surgery_tool/speculum in valid_objects)
	...
	var/obj/item/selection = pick(valid_objects)
...
/mob/proc/get_sound_volume_multiplier()
	if(ear_deaf || sdisabilities & DEAF)
		return 0
	. = 1
