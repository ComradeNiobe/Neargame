...
/client/proc/colorooc()
	...
	if(holder || donation_mycolor.Find(src.ckey))
		...
		var/toggle_textshadow = alert(src, "Do you want your username to glow?", "OOC colour", "Yes", "No")
		if(new_ooccolor)
			prefs.ooccolor = new_ooccolor
			prefs.save_preferences()
		switch(toggle_textshadow)
			if("Yes")
				prefs.nameglow = 1
				prefs.save_preferences()
			if("No")
...
/client/proc/TogglePrivateParty()
	...
	set category = "Server"
	if(private_party)
		message_admins(FONT_LARGE(SPAN_PBOLD("PUBLIC PARTY")))
		usr << "CKEY Whitelist turned off"
		private_party = FALSE
	else
		message_admins(FONT_LARGE(SPAN_PBOLD("PRIVATE PARTY")))
		usr << "CKEY Whitelist turned on"
		private_party = TRUE
