...
/client/proc/cordinates()
	...
	to_chat(usr, "\blue Coordinates: [T.x],[T.y],[T.z]")
	log_debug("Coordinates: [T.x],[T.y],[T.z]")
...
/client/proc/CarbonCopy(atom/movable/O as mob|obj in world)
	...
	message_admins("[key_name_admin(src)] has used CarbonCopy on [O]! (<A HREF='byond://?_src_=holder;adminplayerobservecoodjump=1;X=[usr.x];Y=[usr.y];Z=[usr.z]'>JMP</a>)")
