...
*/
			...
			if(istype(firer, /mob))
				...
				message_admins("ATTACK: [firer] ([firer.ckey])(<A HREF='byond://?_src_=holder;adminplayerobservecoodjump=1;X=[firer.x];Y=[firer.y];Z=[firer.z]'>JMP</A>) shot [M] ([M.ckey]) with [src].") //BS12 EDIT ALG
			else
				M.attack_log += "\[[time_stamp()]\] <b>UNKNOWN SUBJECT (No longer exists)</b> shot <b>[M]/[M.ckey]</b> with a <b>[src]</b>"
				message_admins("ATTACK: UNKNOWN (no longer exists) shot [M] ([M.ckey])(<A HREF='byond://?_src_=holder;adminplayerobservejump=\ref[M]'>JMP</A>) with [src].") //BS12 EDIT ALG
		spawn(0)
			if(A)
				if (!forcedodge)
