...
/datum/reagent/synthflesh/reaction_mob(var/mob/living/M, var/method=TOUCH, var/volume)
	if(!M) M = holder.my_atom
	if(method == TOUCH)
		M.adjustBruteLoss(-(1*volume) * REAGENTS_EFFECT_MULTIPLIER)
		M.adjustFireLoss(-(1*volume) * REAGENTS_EFFECT_MULTIPLIER)
...
/datum/reagent/epinephrine/overdose_process(var/mob/living/M as mob)
	if(prob(33))
		M.adjustHalLoss(5 * REAGENTS_EFFECT_MULTIPLIER)
		M.adjustToxLoss(2 * REAGENTS_EFFECT_MULTIPLIER)
		M.losebreath++
	..()
	return
...
/datum/reagent/strange_reagent/reaction_mob(var/mob/living/carbon/human/M as mob, var/method=TOUCH, var/volume)
	if(M.bruteloss > 80 || M.fireloss > 80)
		...
		if(M.health <= config.health_threshold_dead && !M.suiciding && !mob_ghost && !(NOCLONE in M.mutations))
			...
			M.emote("gasp")
			message_admins("[M.name]/([M.ckey]) was revived using the strange reagent (<A HREF='byond://?_src_=holder;adminplayerobservecoodjump=1;X=[M.x];Y=[M.y];Z=[M.z]'>JMP</a>)")
			M.zombify()
	..()
	return
...
/proc/chemical_mob_spawn(var/datum/reagents/holder, var/amount_to_spawn, var/reaction_name)
	if(holder && holder.my_atom)
		...
		var/message = "A [reaction_name] reaction has occured in [my_area.name]. (<A HREF='byond://?_src_=holder;adminplayerobservecoodjump=1;X=[T.x];Y=[T.y];Z=[T.z]'>JMP</A>)"
		message += " (<A HREF='byond://?_src_=vars;Vars=\ref[A]'>VV</A>)"

		var/mob/M = get(A, /mob)
		if(M)
			message += " - Carried By: [M.real_name] ([M.key]) (<A HREF='byond://?_src_=holder;adminplayeropts=\ref[M]'>PP</A>) (<A HREF='byond://?_src_=holder;adminmoreinfo=\ref[M]'>?</A>)"
		else
...
/datum/reagent/antihol/on_mob_life(var/mob/living/M as mob)
	...
	M.adjustToxLoss(-0.2 * REAGENTS_EFFECT_MULTIPLIER)
	..()
...
/datum/reagent/stimulants/on_mob_life(var/mob/living/M as mob)
	...
	M.AdjustStunned(-1)
	M.AdjustWeakened(-1)
	M.adjustHalLoss(-3 * REAGENTS_EFFECT_MULTIPLIER)
	..()
