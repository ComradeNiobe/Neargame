...
/datum/reagent/nicotine/on_mob_life(var/mob/living/carbon/human/M as mob)
	...
	if(M.has_vice("Smoker"))
		M.viceneed = 0
	M.AdjustStunned(-1)
	M.AdjustWeakened(-1 * REAGENTS_EFFECT_MULTIPLIER)
	..()
	return
...
/datum/reagent/maconha/on_mob_life(var/mob/living/carbon/human/M as mob)
	...
	var/smoke_message = pick("...I feel good.", "...I... Feel relaxed", "... Oh oh Oh! ...", "... Toke out the pain ....")
...
/datum/reagent/maconha/on_remove(data)
	...
	for(var/obj/ripple_controller/R in M?.client?.screen)
		...
		qdel(R) //just to be ssure it will disappear
...
/datum/reagent/crank/overdose_process(var/mob/living/M as mob)
	M.adjustBrainLoss(rand(1,10) * REAGENTS_EFFECT_MULTIPLIER)
	M.adjustToxLoss(rand(1,10) * REAGENTS_EFFECT_MULTIPLIER)
	M.adjustBruteLoss(rand(1,10) * REAGENTS_EFFECT_MULTIPLIER)
	..()
	return
...
/datum/reagent/krokodil/overdose_process(var/mob/living/M as mob)
	if(prob(10))
		M.adjustBrainLoss(rand(1,5) * REAGENTS_EFFECT_MULTIPLIER)
		M.adjustToxLoss(rand(1,5) * REAGENTS_EFFECT_MULTIPLIER)
	..()
	return
...
/datum/reagent/krokodil/addiction_act_stage4(var/mob/living/carbon/human/M as mob)
	if(!(SKELETON in M.mutations))
		...
		M.adjustBruteLoss(rand(50,80) * REAGENTS_EFFECT_MULTIPLIER) // holy shit your skin just FELL THE FUCK OFF
		if(ishuman(M))
...
/datum/reagent/fakeweed
	name = "JWH-07"
	id = "fakeweed"
	description = "An extremely dangerous synthetic variant of cannabis."
	reagent_state = LIQUID
	color = "#558833" // rgb: 96, 165, 132
	overdose_threshold = 25
	addiction_threshold = 10 // You'll get hooked.
	var/valueAddIT
	var/valueAddST
	var/valueAddDX

/datum/reagent/fakeweed/on_mob_life(var/mob/living/carbon/human/M as mob)
	if(!M)
		M = holder.my_atom
	if(!istype(M))
		holder.remove_reagent(src.id, metabolization_rate)
	...
	if(first_life)
		first_life = FALSE
		if(M.special == "weedstrong")
			M.my_stats.add_mod("maconha", stat_list(ST = 2, HT = 1, DX = -2, IN = 3), time = 200) // If you're a toke-guy, you don't suffer any ill effects
		else
			M.my_stats.add_mod("maconha", stat_list(ST = -1, DX = -2, IN = -2), time = 200) // No positives.
...
/datum/reagent/fakeweed/on_remove(data)
	..()
	var/mob/living/carbon/human/M = holder?.my_atom
	if(!istype(M))
		return
