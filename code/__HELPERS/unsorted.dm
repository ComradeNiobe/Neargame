...
/mob/proc/rename_self(var/role, var/allow_numbers=0)
	spawn(0)
		...
		for(var/i=1,i<=3,i++)	//we get 3 attempts to pick a suitable name.
			...
			newname = sanitize_name(newname, MAX_MESSAGE_LEN, TRUE, FALSE)	//returns null if the name doesn't meet some basic requirements. Tidies up a few other things like bad-characters.
...
/proc/do_mob(var/mob/user , var/mob/target, delay as num) //This is quite an ugly solution but i refuse to use the old request system.
	...
	for (var/i = 1 to 16)
		...
		if(user.stat || user.weakened || user.stunned)
			...
			user.doing_after = FALSE
			return 0
		if (get_turf(user) != user_loc || get_turf(target) != target_loc || user.get_active_hand() != holding)
			qdel(progbar)
			user.doing_after = FALSE
			return 0
...
/proc/do_after(var/mob/user as mob, delay as num, var/numticks = 10, var/needhand = 1)
	...
	for (var/i = 1 to 16)
		...
		if(QDELETED(user) || user.stat || user.weakened || user.stunned || !(user.loc == T))
			qdel(progbar)
			user.doing_after = FALSE
			return 0
...
//						X.sd_set_opacity(!X.opacity)			//TODO: rewrite this code so it's not messed by lighting ~Carn
	...
	for(var/obj/O as anything in doors)
