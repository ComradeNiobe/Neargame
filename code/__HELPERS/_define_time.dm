...
#define __extern__timeofhour text2num(call_ext(__btime__libName, "gettime")())
/hook/startup/proc/checkbtime()
	try
		...
		log_world("PRECISE_TIMER_AVAILABLE is defined in btime.dm, but calling the btime library failed: [e]")
		log_world("This is a fatal error. The world will now shut down.")
		qdel(world)
#else
